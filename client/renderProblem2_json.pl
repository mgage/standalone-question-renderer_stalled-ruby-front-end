...
#############################################
 ...
 use constant  TEMPOUTPUTFILE   => 'renderProblemOutput1.html'; 
...
my $input2 = $xmlrpc_client->setInputTable();
my $input3 = {%$input2, %$input};
warn "Please enter the name of the file to be rendered:  renderProblem2_json.pl    fileName" unless defined $fileName and $fileName;
...
# }
  my $json_client = new JSON::RPC::Client;
  
  my $callobj = {
      method  => 'renderProblem',
      params  => $input3,     # ex.) params => { a => 20, b => 10 } for JSON-RPC v1.1
   };
 my $res = $json_client->call($XML_URL, $callobj);
 ...
 if($res) {
      ...
      print $json_client->status_line;
   }
my $fname = "data_server_to_client.txt";


# 
# # output for file
# local(*FH);
# open(FH, ">$fname") or die "Can't open file $fname for writing";
# print FH '$rh = {\n';
# print FH pretty_print2_rh($result);
# print FH '}';
# close(FH);
# local(*FH);
# 
