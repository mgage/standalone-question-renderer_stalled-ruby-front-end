...
sub pre_header_initialize {
	...
	local(*FH);
    open(FH, ">/Users/mgage1/webwork/renderProblem_data2.txt") 	or die "cannot open  renderProblem_data2.txt: $!";
    print FH "form sent to server from browser";
    print FH WebworkClient::pretty_print($r->{paramcache});
    print FH %{$r->{paramcache}};
    close(FH);
